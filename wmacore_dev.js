...
var wma_tilesets = [
  ...
  },
  {
    ...
    getTileURL: function(y, x, z)
    {
      ...
      return wma_imgbase + 'blue_marble/'+(z+3)+'/'+y+'/'+x1+'.jpg';
    },
  ...
  }, */
  {
    ...
    getTileURL: function(y, x, z)
    {
      ...
      return wma_imgbase + 'lro_moon/lromoon_' + ('00' + (5-z)) + '_' +
             ('000' + x1).substr(-3) + '_'+('000'+y).substr(-3) + '.png';
    },
  ...
  },
  {
    ...
    getTileURL: function(y, x, z)
    {
      ...
      return wma_imgbase + 'mars/mars_' + ('00' + (5 - z)) + '_' +
             ('000' + x1).substr(-3) + '_' + ('000' + y).substr(-3) + '.png';
    },
    linkcolor: [ "white", "black 0pt 0pt 2pt" ],
  ...
  },
  {
    ...
    getTileURL: function(y, x, z)
    {
      ...
      return wma_imgbase + 'venus/venus_' + ('00' + (3 - z)) + '_' +
             ('000' + x1).substr(-3) + '_' + ('000' + y).substr(-3) + '.png';
    },
    linkcolor: [ "white", "1px 0px 2px black, 0px -1px 2px black, 0px 1px 2px black, -1px 0px 2px black" ],
  ...
  },
  {
    ...
    getTileURL: function(y, x, z)
    {
      ...
      if (x1 < 0)
        x1 += wma_zoomsize[z] * 2;
      var z1 = 6 - z;
      return wma_imgbase + 'mercury/' + ((z1 < 3) ? (z1 + '/') : '') +
             ((z1 == 0) ? (Math.floor(x1 / 100) + '/') : '') + 'merc_' +
             ('00' + z1) + '_' + ('000' + x1).substr(-3) + '_' +
             ('000' + y).substr(-3) + '.png';
    },
    linkcolor: [ "white", "1px 0px 2px black, 0px -1px 2px black, 0px 1px 2px black, -1px 0px 2px black" ],
  ...
  },
  {
    ...
    getTileURL: function(y, x, z)
    {
      ...
      return wma_imgbase + 'io/io_' + ('00' + (4 - z)) + '_' +
             ('000' + x1).substr(-3) + '_' + ('000' + y).substr(-3) + '.png';
    },
    linkcolor: [ "black", "1px 0px 2px white, 0px -1px 2px white, 0px 1px 2px white, -1px 0px 2px white" ],
  ...
  },
  {
    ...
    getTileURL: function(y, x, z)
    {
      ...
      return wma_imgbase + 'titan/titan_' + ('00'+(3-z)) + '_' +
             ('000' + x1).substr(-3) + '_' + ('000' + y).substr(-3) + '.png';
    },
    linkcolor: [ "black", "1px 0px 2px white, 0px -1px 2px white, 0px 1px 2px white, -1px 0px 2px white" ],
...
function wikiminiatlasInstall(wma_widget, url_params)
{
  ...
  var wmasize = {},
      wmakml = {
        shown: false, drawn: false,
        canvas: null, c: null,
        ways: null, areas: null,
  ...
  function setupWidget()
  {
    ...
    if (coord_filter.test(coord_params))
    {
      ...
      if (coord_filter.test(coord_params))
      {
        ...
        wmaSetZoom(Math.min(wma_zoom,
          (wma_tileset != 0 ?
            wma_tilesets[wma_tileset].maxzoom :
            ((hasCanvas && wma_highzoom_activated) ? 15 : 12)
          )
        ));
    ...
    var l = strings.sover[UILang].list;
    ...
    labelcaption = $('<div></div>').css(
      {
        position: 'absolute', top: '30px', left: '60px',
        zIndex: 100, fontSize: '40px', color: 'white',
        textShadow: '1px 1px 5px black', fontWeight: 'bold'
      }
    ).appendTo('#wma_widget');
  ...
  function wmaNewTile()
  {
    var d = $('<div></div>')
      ...
      .click(function(e) {
        ...
        if (r.x != wma_mdcoord.x || r.y != wma_mdcoord.y || !t.csrender)
          return true;
  ...
  function wmaResize()
  {
    ...
    if (hasCanvas) {
      ...
      // set webgl viewport
      if (bldg3dc.viewport) {
        bldg3dc.viewport(0, 0, nw*wma_dpr, nh*wma_dpr);
      }
  ...
  {
    var i, j, n, c = wmasize.c, w = wmasize.ways, a = wmasize.areas,
        p , gx = wma_gx, gy = wma_gy;
  ...
  function moveWikiMiniAtlasMapTo()
  {
    ...
    if (wma_gy < 0)
      wma_gy = 0;
    if (wma_gx < 0)
      wma_gx += Math.floor(wma_zoomsize[wma_zoom] * 2 * tsx); // TODO: Mercator is 1:1 not 2:1
    if (wma_gx > 0)
      wma_gx %= Math.floor(wma_zoomsize[wma_zoom] * 2 * tsx);
      ...
      for (i = 0; i < wma_nx; i++)
      {
        ...
        if (thistile.url != tileurl || wma_highzoom_purge)
        {
          ...
          else
          {
            ...
            // TODO
            (function(hash, turl, lo)
            {
              // closure to retain access to dataurl in sucess callback
  ...
  function updateMarker(m)
  {
    ...
    if (newx < -100)
     newx += (wma_zoomsize[wma_zoom] * 256);
  ...
  function wmaKeypress(ev)
  {
    ...
    switch(ev.keyCode || ev.which)
    {
      ...
      case 76 :
        var ns, n, alllang = ['en', 'de', 'fr', 'ru', 'ar', 'he', 'ko', 'ja', 'ml', 'fa', 'commons'];
  ...
  {
    return wma_site + '_' +
           (wma_zoomsize[z]-y-1) + '_' +
           (x % (wma_zoomsize[z]*2)) + '_' +
           z + '_' + wma_tilesets[wma_tileset].globe;
  }

  function wmaGetDataURL(y, x, z)
  {
    return wma_database + '?l=' + wma_site + '&a=' + (wma_zoomsize[z]-y-1) +
           '&b=' + (x % (wma_zoomsize[z]*2)) + '&z=' + z +
           '&g=' + wma_tilesets[wma_tileset].globe;
  }
  ...
  function wmaSelectTileset(n, init)
  {
    ...
    if (newz < wma_tilesets[n].minzoom) {
      newz = wma_tilesets[n].minzoom;
    }
  ...
  {
    return {
      lat: 180.0 * (0.5 - y / (wma_zoomsize[wma_zoom] * 128)),
      lon: 360.0 * (x / (wma_zoomsize[wma_zoom] * 256))
    };
  }
  ...
  {
    var fs = window.open('', 'showwin', 'left=0, top=0, width=' + screen.width  + ', height=' +
                         screen.height + ', toolbar=0, resizable=0, fullscreen=1'),
        page = url_params.page,
        lang = url_params.lang,
  ...
  function wmaReceiveMessage(e)
  {
    ...
    if (('minlon' in d) && ('maxlon' in d)) {
      if (!('maxlon' in wmakml) || d.maxlon > wmakml.maxlon) {
        wmakml.maxlon = d.maxlon;
      }
      if (!('minlon' in wmakml) || d.minlon < wmakml.minlon) {
        wmakml.minlon = d.minlon;
      }
    }
    ...
    if ('getcoords' in d)
    {
      ...
      if (window.parent)
        window.parent.postMessage(JSON.stringify(gci), e.origin);
    }
    ...
    if ('moveto' in d) {
      ...
      if ('lat' in t && 'lon' in t) {
        ...
        if (wmaGlobe) {
          wmaGlobe.setLatLon(t.lat, t.lon);
        }
      }
    }
  }
  ...
  function processSizeOverlay(d)
  {
    ...
    if (canvas) {
      wmasize = {
        shown: true, drawn: false, canvas: canvas,
        c: c, ways: null, areas: null,
        maxlat: -Infinity, minlat: Infinity
      };
    } else {
      wmasize = {
        shown: true, drawn: false, canvas: null,
        c: null, ways: null, areas: null,
        maxlat: -Infinity, minlat: Infinity
      };
    ...
    // postprocess data
    var a = wmasize.areas, w = wmasize.ways,
        clon = (wmasize.maxlon + wmasize.minlon) / 2.0,
        clat = (wmasize.maxlat + wmasize.minlat) / 2.0;
    function norm(w)
    {
      var k, dx = 0.0;
      if (w[0].lon < wmasize.minlon)
        dx = 360.0;
...
wmaMenu.prototype.addCheck = function(html, func, selected, rtl)
{
  function check() {
    if (selected) {
      item.addClass('wmamenuchecked');
    } else {
      item.removeClass('wmamenuchecked');
    }
  }
...
wmaMenu.prototype.addGroup = function(options, func, selected, rtl)
{
  ...
  function select(n) {
    ...
    for (var i in items) {
      if (i == n) {
        items[i].addClass('wmamenuselected');
      } else {
        items[i].removeClass('wmamenuselected');
      }
    }
    current = n;
  }
...
wmaMenu.prototype.toggle = function() { if (this.shown) { this.hide(); } else { this.show(); } };
wmaMenu.prototype.close = function() { this.hide(); if (this.parent) { this.parent.close(); } };
