var $jscomp=$jscomp||{};$jscomp.scope={};$jscomp.findInternal=function(f,d,e){f instanceof String&&(f=String(f));for(var w=f.length,x=0;x<w;x++){var T=f[x];if(d.call(e,T,x,f))return{i:x,v:T}}return{i:-1,v:void 0}};$jscomp.ASSUME_ES5=!1;$jscomp.ASSUME_NO_NATIVE_MAP=!1;$jscomp.ASSUME_NO_NATIVE_SET=!1;$jscomp.SIMPLE_FROUND_POLYFILL=!1;$jscomp.ISOLATE_POLYFILLS=!1;$jscomp.FORCE_POLYFILL_PROMISE=!1;$jscomp.FORCE_POLYFILL_PROMISE_WHEN_NO_UNHANDLED_REJECTION=!1;
...
$jscomp.polyfill=function(f,d,e,w){d&&($jscomp.ISOLATE_POLYFILLS?$jscomp.polyfillIsolated(f,d,e,w):$jscomp.polyfillUnisolated(f,d,e,w))};$jscomp.polyfillUnisolated=function(f,d,e,w){e=$jscomp.global;f=f.split(".");for(w=0;w<f.length-1;w++){var x=f[w];if(!(x in e))return;e=e[x]}f=f[f.length-1];w=e[f];d=d(w);d!=w&&null!=d&&$jscomp.defineProperty(e,f,{configurable:!0,writable:!0,value:d})};
$jscomp.polyfillIsolated=function(f,d,e,w){var x=f.split(".");f=1===x.length;w=x[0];w=!f&&w in $jscomp.polyfills?$jscomp.polyfills:$jscomp.global;for(var T=0;T<x.length-1;T++){var S=x[T];if(!(S in w))return;w=w[S]}x=x[x.length-1];e=$jscomp.IS_SYMBOL_NATIVE&&"es6"===e?w[x]:null;d=d(e);null!=d&&(f?$jscomp.defineProperty($jscomp.polyfills,x,{configurable:!0,writable:!0,value:d}):d!==e&&(void 0===$jscomp.propertyToPolyfillSymbol[x]&&($jscomp.propertyToPolyfillSymbol[x]=$jscomp.IS_SYMBOL_NATIVE?$jscomp.global.Symbol(x):
$jscomp.POLYFILL_PREFIX+x),x=$jscomp.propertyToPolyfillSymbol[x],$jscomp.defineProperty(w,x,{configurable:!0,writable:!0,value:d})))};$jscomp.polyfill("Array.prototype.find",function(f){return f?f:function(d,e){return $jscomp.findInternal(this,d,e).v}},"es6","es3");
$jscomp.polyfill("Array.prototype.fill",function(f){return f?f:function(d,e,w){var x=this.length||0;0>e&&(e=Math.max(0,x+e));if(null==w||w>x)w=x;w=Number(w);0>w&&(w=Math.max(0,x+w));for(e=Number(e||0);e<w;e++)this[e]=d;return this}},"es6","es3");$jscomp.typedArrayFill=function(f){return f?f:Array.prototype.fill};$jscomp.polyfill("Int8Array.prototype.fill",$jscomp.typedArrayFill,"es6","es5");$jscomp.polyfill("Uint8Array.prototype.fill",$jscomp.typedArrayFill,"es6","es5");
...
var lc={},wma_tilesets=[{name:"mapFull",globe:"Earth",credit:' / Map data \u00a9 <a href="http://www.openstreetmap.org">OpenStreetMap</a> contributors, <a href="http://www.opendatacommons.org/licenses/odbl">License</a>',getTileURL:function(f,d,e,w){me=wma_tilesets[0];var x=7<e?"mapnik/":"mapnik.new/";return w||"https:"==document.location.protocol?7<=e?wma_imgbase+x+e+"/"+f+"/tile_"+f+"_"+d%(2*wma_zoomsize[e])+".png":wma_imgbase+x+e+"/tile_"+f+"_"+d%(2*wma_zoomsize[e])+".png":7<=e?"//"+(d+f)%8+wma_tilebase+
x+e+"/"+f+"/tile_"+f+"_"+d%(2*wma_zoomsize[e])+".png":"//"+(d+f)%8+wma_tilebase+x+e+"/tile_"+f+"_"+d%(2*wma_zoomsize[e])+".png"},linkcolor:["#2255aa","1px 0px 2px white, 0px -1px 2px white, 0px 1px 2px white, -1px 0px 2px white"],equator:40075,maxzoom:20,minzoom:0},{name:"mapPhysical",globe:"Earth",getTileURL:function(f,d,e){return wma_imgbase+"relief/"+e+"/"+f+"_"+d%(2*wma_zoomsize[e])+".png"},linkcolor:["#2255aa","white 0pt 0pt 2pt"],equator:40075,maxzoom:5,minzoom:0},{name:"mapLandsat",globe:"Earth",
...
return b}function x(){var a,b;if(null===ha){$(document).on("mousemove touchmove",function(c){window.scrollTo(0,0);var h=sa(c);null!==ta&&(F-=h.x-ta.x,I-=h.y-ta.y,ta=h,P(),ba&&(ba=!1,xa()));J.shown&&Ha(ca(F+h.x,I+h.y));h=ca(F+H/2,I+G/2);D&&D.setLatLon(h.lat,h.lon);c.preventDefault()}).on("mouseup touchend",function(){ta=null});$("#wma_map").on("dblclick",gb).on("mousedown touchstart",Sa);ha=document.getElementById("wma_map");ia=Math.floor(H/128)+2;na=Math.floor(G/128)+2;ja=[];for(b=0;b<na;++b)for(a=
0;a<ia;++a)ja.push(w());Q.obj=$('<div id="wmamarker"></div>');$(ha).append(Q.obj)}}function T(){var a=$(window).width(),b=$(window).height(),c=Math.floor(a/128)+2,h=Math.floor(b/128)+2,l;H=a;G=b;Y&&(null!==B.canvas&&(B.canvas.attr({width:a*N,height:b*N}).css({width:a+"px",height:b+"px"}),B.c.setTransform(N,0,0,N,0,0)),bldg3d.attr({width:a*N,height:b*N}).css({width:a+"px",height:b+"px"}),bldg3dc.viewport&&bldg3dc.viewport(0,0,a*N,b*N));if(c!=ia||h!=na){ia=c;for(na=h;ja.length<c*h;)ja.push(w());for(l=
0;l<ja.length;++l)l<c*h?ja[l].div.show():ja[l].div.hide();P();$(ha).width(a).height(b)}else B.shown&&ka()}function S(){B.shown?B.canvas.fadeOut(200):B.canvas.fadeIn(200);B.shown=!B.shown}function Ha(a){function b(z){var q;if(0<z.length){var v=z[0].lat+a.lat;v=ya(v,z[0].lon/Math.cos(v/180*Math.PI)+a.lon);l.moveTo(v.x-g,v.y-k);for(q=1;q<z.length;++q)v=z[q].lat+a.lat,v=ya(v,z[q].lon/Math.cos(v/180*Math.PI)+a.lon),l.lineTo(v.x-g,v.y-k)}}var c,h,l=J.c,y=J.ways,n=J.areas,g=F,k=I;if(null!==l){l.clearRect(0,
...
l.stroke()}}}function ka(a,b,c,h,l,y){function n(t){var m,p=0;t[0].lon<B.minlon&&(p=2*a);if(0<t.length){var L=ya(t[0].lat,t[0].lon,a);var K=L.x;y.moveTo(L.x-z+p,L.y-c);for(m=1;m<t.length;++m)L=la(t[m].lat,t[m].lon,a),K=L.x-K,Math.abs(K)>a&&(p-=2*Math.round(K/(2*a))*a),K=L.x,y.lineTo(L.x-z+p,L.y-c)}}var g=B.ways,k=B.areas;a=a||128*wma_zoomsize[C];b=void 0!==b?b:F;c=void 0!==c?c:I;y=y||B.c;var z=b;if(null!==y)for(h=h||B.canvas[0].width,l=l||B.canvas[0].height,y.clearRect(0,0,h,l),l=-2;2>=l;++l){z=b+
2*l*a;var q=ya(0,B.minlon,a);var v=ya(0,B.maxlon,a);if(!(0>v.x-z||q.x-z>h)){if(null!==k)for(y.fillStyle="rgb(255, 0, 0)",y.strokeStyle="rgb(0, 0, 0)",y.lineWidth=2,q=0;q<k.length;++q){y.globalCompositeOperation="source-over";for(v=0;v<k[q].outer.length;++v)y.beginPath(),n(k[q].outer[v]),y.closePath(),y.fill(),y.stroke();y.globalCompositeOperation="destination-out";for(v=0;v<k[q].inner.length;++v)y.beginPath(),n(k[q].inner[v]),y.closePath(),y.fill(),y.stroke()}if(null!==g){y.globalCompositeOperation=
"source-over";y.lineWidth=4;y.strokeStyle="rgb(0, 0, 255)";y.beginPath();for(q=0;q<g.length;++q)n(g[q]);y.stroke()}}}}function P(){function a(K,u){function da(ea,V,za,Ia,Ja){ea.click(function(Aa){var W=sa(Aa);if(W.x!=pa.x||W.y!=pa.y)Aa.preventDefault();else if(1==Aa.which&&!Aa.ctrlKey){null==Z.panel&&hb();var qa=H-30,Ta=G-80;W=za;var Da=Ia;W>qa&&(Da=Math.round(Da*qa/W),W=qa);Da>Ta&&(W=Math.round(W*Ta/Da));Z.span.find("img").remove();qa=$("<img/>",{id:"wma_wmaci_image"}).appendTo(Z.span);2*W<za?qa.attr({src:Ba(V,
10*Math.floor(W/10),Ja),srcset:Ba(V,20*Math.floor(W/10),Ja)+" 2x"}):W<za?qa.attr({src:Ba(V,10*Math.floor(W/10),Ja)}):qa.attr("src","//commons.wikimedia.org/wiki/Special:FilePath/"+V);Z.link.attr("href","//commons.wikimedia.org/wiki/File:"+V).text("[[:commons:File:"+decodeURIComponent(V)+"]]");Z.panel.fadeIn(200);Z.shown=!0;Aa.preventDefault()}}).attr("href","https://commons.wikimedia.org/wiki/File:"+V)}var A,fa=[0,0,5,0,0,2,3,4,5,6,6],Ea=[0,0,8,0,0,2,3,4,5,6,6];K.text("");for(A=0;A<u.length;++A){var M=
$("<a></a>");if(!(8>=C&&!u[A].lang&&u[A].wg<wma_median_weights.commons[C])){if("img"in u[A]){da(M,u[A].img,u[A].w,u[A].h,u[A].m5);var R=parseInt(u[A].w)>parseInt(u[A].h)?-2==u[A].style?24:48:Math.floor((-2==u[A].style?24:48)*u[A].w/u[A].h);var ma=Math.floor(R/u[A].w*u[A].h);M.addClass("cthumb").append($("<img/>",{title:decodeURIComponent(u[A].img)}).error(function(){$(this).parent().hide()}).attr({src:Ba(u[A].img,R,u[A].m5),srcset:2*R<=u[A].w?Ba(u[A].img,2*R,u[A].m5)+" 2x":""}).css({width:R+"px",
height:ma+"px"}));18>u[A].head?(M.addClass("dir dir"+u[A].head),R=8):R=6;M.css({top:u[A].ty-R+"px",left:u[A].tx-R+"px"})}else M.addClass("label").addClass("label"+u[A].style).addClass("label-"+u[A].lang).css(Ka).attr({href:"https://"+u[A].lang+".wikipedia.org/wiki/"+u[A].page,target:"_top"}).css({top:u[A].ty-Ea[u[A].style]+"px",left:u[A].tx-fa[u[A].style]+"px",direction:isRTL(u[A].lang)?"rtl":"ltr"}),(R=/(.*?)(\s*\([^\)]*\)$|\s*, [^, \(]*$)/.exec(u[A].name))?M.text(R[1]).append($('<span class="hell">&hellip;</span>')).append($('<span class="sufx"></span>').text(R[2]).hide()).hover(function(){$(".hell",
this).hide();$(".sufx",this).show()},function(){$(".hell",this).show();$(".sufx",this).hide()}):M.text(u[A].name);K.append(M)}}}function b(K,u){var da=K.lx,A=K.ly,fa=1<<K.lz-wma_maxlabel,Ea=[],M,R=[[],[],[],[]];for(M=0;M<u.length;++M){var ma=128*(da/fa-u[M].dx);var ea=128*(A/fa-(wma_zoomsize[wma_maxlabel]-u[M].dy-1));ma=(u[M].tx+u[M].fx-ma)*fa;var V=(u[M].ty+u[M].fy-ea)*fa;0<=ma&&128>ma&&0<=V&&128>V&&(ea=jQuery.extend(!0,{},u[M]),ea.tx=Math.floor(ma),ea.ty=Math.floor(V),ea.fx=ma-ea.tx,ea.fy=V-ea.ty,
R[Math.floor(ma/64)+2*Math.floor(V/64)].push(ea))}for(M=0;4>M;++M)0<R[M].length&&(R[M].sort(function(za,Ia){return Ia.wg-za.wg}),Ea.push(R[M][0]));a(K.span,Ea)}0>I&&(I=0);0>F&&(F+=Math.floor(256*wma_zoomsize[C]));0<F&&(F%=Math.floor(256*wma_zoomsize[C]));var c=Math.floor(F/128)%ia,h=Math.floor(I/128)%na,l=F%128,y=I%128,n,g,k;ib();for(g=0;g<na;g++)for(n=0;n<ia;n++){var z=(n+c)%ia+(g+h)%na*ia;var q=Math.floor(F/128)+n;var v=Math.floor(I/128)+g;var t=wma_tilesets[O].getTileURL(v,q,C);var m=ja[z];m.div.css({left:128*
n-l+"px",top:128*g-y+"px"});if(m.url!=t||wma_highzoom_purge){m.url=t;if(0==O&&12<C){if(m.csrender||(m.csrender=!0,m.img.fadeOut(200)),m.csx!=q||m.csy!=v||m.csz!=C||wma_highzoom_purge)m.can.hide(),m.img.hide(),wmajt.update(q,v,C,m,wma_highzoom_purge)}else{m.csz=C;m.csrender&&m.can.fadeOut(200);if(v>=wma_zoomsize[C]){m.img.fadeOut(0);m.span.text("");continue}m.img.attr("src")!=t?m.img.fadeOut(0).attr("src",t):m.csrender&&m.img.fadeIn(200);m.csrender=!1}null!==m.xhr&&m.xhr.abort();m.lx=q;m.ly=v;m.lz=
C;m.span.html('<span class="loading">'+strings.labelLoading[E]+"</span>");if(C>wma_maxlabel){var p=Math.floor(q/(1<<C-wma_maxlabel));t=Math.floor(v/(1<<C-wma_maxlabel));z=Ua(t,p,wma_maxlabel);t=La(t,p,wma_maxlabel)}else z=Ua(v,q,C),t=La(v,q,C);if(!lc[t]||"filler"in lc[t]){if(lc[t]&&"filler"in lc[t])a(m.span,lc[t].label);else if(C<=wma_maxlabel&&0<C){var L=La(Math.floor(v/2),Math.floor(q/2),C-1);if(lc[L]){p=q%2;lc[t]={label:[],filler:1};for(k=0;k<lc[L].label.length;k++)Math.floor(lc[L].label[k].tx/
64)===q%2&&Math.floor(lc[L].label[k].ty/64)===v%2&&(p=jQuery.extend(!0,{},lc[L].label[k]),p.tx=2*(p.tx+p.fx)-q%2*128,p.ty=2*(p.ty+p.fy)-v%2*128,p.fx=p.tx-Math.floor(p.tx),p.fy=p.ty-Math.floor(p.ty),p.tx=Math.floor(p.tx),p.ty=Math.floor(p.ty),lc[t].label.push(p));a(m.span,lc[t].label)}}(function(K,u,da){m.xhr=$.ajax({url:u,context:m}).success(function(A){try{lc[K]=JSON.parse(A),da?a(this.span,lc[K].label):b(this,lc[K].label)}catch(fa){this.span.html(A)}}).error(function(){this.span.text("")})})(t,
z,C<wma_maxlabel)}else C<wma_maxlabel?a(m.span,lc[t].label):b(m,lc[t].label)}}Ma(Q);for(z=0;z<Na.length;++z)Ma(Na[z]);wma_highzoom_purge=!1;C>=wma_bldg3dzoom&&update3dBuildings();ka()}function X(a){a.clearColor(0,0,0,0);a.clearDepth(1);a.enable(a.DEPTH_TEST);a.depthMask(!0);a.depthFunc(a.LEQUAL);a.clear(a.COLOR_BUFFER_BIT+a.DEPTH_BUFFER_BIT);var b=a.createProgram(),c=a.createShader(a.VERTEX_SHADER);a.shaderSource(c,$("#bldg-vs").text());a.compileShader(c);a.attachShader(b,c);c=a.createShader(a.FRAGMENT_SHADER);
...
b);return function(){var y=H/2,n=G/2,g=60/(128<<C),k=ca(F+y,I+n);180<k.lon&&(k.lon-=360);a.uniform3f(h,y*g,n*g,850);a.uniform3f(l,k.lon,k.lat,0);a.clear(a.COLOR_BUFFER_BIT|a.DEPTH_BUFFER_BIT);wmajt.renderWebGLBuildingData()}}function ra(){function a(m){var p;if(0<q)for(n.moveTo((m[0][0]-k.lon)*v+h,l-(m[0][1]-k.lat)*v),p=1;p<m.length;p++)n.lineTo((m[p][0]-k.lon)*v+h,l-(m[p][1]-k.lat)*v);for(p=0;p<m.length-1;p++)n.moveTo((m[p][0]-k.lon)*v+h,l-(m[p][1]-k.lat)*v),n.lineTo((m[p][0]-k.lon)*t+h,l-(m[p][1]-
k.lat)*t),n.lineTo((m[p+1][0]-k.lon)*t+h,l-(m[p+1][1]-k.lat)*t)}if(Y&&C>wma_bldg3dzoom){var b=wmajt.ref_z(),c=wmajt.zbuild(),h=H/2,l=G/2,y=(128<<C)/60,n=bldg3dc,g,k=ca(F+h,I+l);180<k.lon&&(k.lon-=360);n.lineWidth=.5;n.strokeStyle="rgb(0, 0, 0)";n.beginPath();bldg3dc.clearRect(0,0,H,G);for(g in b){var z=c[g].geo.coordinates;b=3*c[g].tags["building:levels"]||c[g].tags.height;var q=3*c[g].tags["building:min_level"]||c[g].tags.min_height||0;var v=y*(1+q/450);var t=y*(1+b/450);if("Polygon"===c[g].geo.type)for(g=
0;g<z.length;g++)a(z[g]);else a(z)}n.stroke();bldg3d.show();null!==bldg3dtimer&&clearTimeout(bldg3dtimer);setTimeout(update3dBuildings,1500)}else bldg3d.hide()}function Ma(a){var b=la(a.lat,a.lon),c=b.x-F;-100>c&&(c+=256*wma_zoomsize[C]);a.obj.css("left",c-6+"px");a.obj.css("top",b.y-I-6+"px")}function Sa(a){pa=ta=sa(a)}function gb(a){a=sa(a);F+=a.x-H/2;I+=a.y-G/2;ba&&(ba=!1,xa());P();return!1}function fb(a){Oa&&clearTimeout(Oa);Fa.stop(!0).fadeTo(1,1).text(a);Oa=setTimeout(function(){Fa.fadeOut(200)},
...
"lat"in a&&"lon"in a&&(Pa(a.lat,a.lon),D&&D.setLatLon(a.lat,a.lon)))}function bb(a,b){function c(g){var k,z=180/Math.PI,q=Math.PI/2,v=180/2.003750834E7,t=[],m=-Infinity,p=Infinity;for(k=0;k<g.length;++k){var L=g[k][0]*v;var K=z*(2*Math.atan(Math.exp(g[k][1]*v/z))-q);L>m&&(m=L);L<p&&(p=L);K>b.maxlat&&(b.maxlat=K);K<b.minlat&&(b.minlat=K);t.push({lat:K,lon:L})}"maxlon"in b&&"maxlon"in b?(g=[{r:Math.max(b.maxlon,m),l:Math.min(b.minlon,p)},{r:Math.max(b.maxlon,m+360),l:Math.min(b.minlon,p+360)},{r:Math.max(b.maxlon+
360,m),l:Math.min(b.minlon+360,p)}],g.sort(function(u,da){return u.r-u.l-(da.r-da.l)}),b.minlon=g[0].l,b.maxlon=g[0].r):(b.maxlon=m,b.minlon=p);return t}function h(g){var k={outer:[],inner:[]},z;k.outer.push(c(g[0]));for(z=1;z<g.length;z++)k.inner.push(c(g[z]));b.areas?b.areas.push(k):b.areas=[k]}function l(g){g=[c(g)];b.ways?b.ways.push.apply(b.ways,g):b.ways=g}function y(g){var k;switch(g.type){case "LineString":l(g.coordinates);break;case "MultiLineString":for(k=0;k<g.coordinates.length;k++)l(g.coordinates[k]);
break;case "Polygon":h(g.coordinates);break;case "MultiPolygon":for(k=0;k<g.coordinates.length;k++)h(g.coordinates[k])}}var n;if("type"in a)if("GeometryCollection"==a.type)for(n=0;n<a.geometries.length;++n)y(a.geometries[n]);else y(a)}function ob(a){bb(a,B);if(B.ways||B.areas)Ra(),ka();if(B.maxlon&&B.minlon&&-Infinity<B.maxlat&&Infinity>B.minlat){a=(B.maxlon+B.minlon)/2;var b=(B.maxlat+B.minlat)/2,c=(B.maxlon-B.minlon)/180*384,h=(B.maxlat-B.minlat)/180*384,l;for(l=0;12>l&&!(c>H/2||h>G/2);++l)c*=2,
h*=2;aa(l);Pa(b,a)}}function jb(a){function b(n){var g,k=0;n[0].lon<J.minlon&&(k=360);for(g=0;g<n.length;++g)n[g].lon-=l-k,n[g].lon*=Math.cos(n[g].lat/180*Math.PI),n[g].lat-=y}var c=J.c,h=J.canvas;h?J={shown:!0,drawn:!1,canvas:h,c:c,ways:null,areas:null,maxlat:-Infinity,minlat:Infinity}:(J={shown:!0,drawn:!1,canvas:null,c:null,ways:null,areas:null,maxlat:-Infinity,minlat:Infinity},Ra(J));bb(a,J);a=J.areas;c=J.ways;var l=(J.maxlon+J.minlon)/2,y=(J.maxlat+J.minlat)/2;if(null!==a)for(i=0;i<a.length;++i){for(j=
...
maxlat:-Infinity,minlat:Infinity},Ka={},D=!1,Za=null,Y="HTMLCanvasElement"in window,N="devicePixelRatio"in window?window.devicePixelRatio:1,Fa,Oa=null;(function(){var a=d.wma||window.location.search.substr(1);var b=d.page,c=d.lang;va=d.globe||"Earth";b&&Y&&$.ajax({url:"//tools.wmflabs.org/wiwosm/osmjson/getGeoJSON.php?lang="+c+"&article="+b,dataType:"json",success:ob});Za=function(){if(!Y)return function(){};var g=$("<canvas></canvas>").attr({width:2048,height:1024}).css({display:"none"}),k=$("<canvas></canvas>").attr({width:1536,
height:768}).css({display:"none"}),z=$("<canvas></canvas>").attr({width:1536,height:768}).css({display:"none"}),q=$("<div></div>").css({position:"absolute",width:"80px",height:"80px",bottom:"20px",right:"5px",zIndex:50,display:"none",borderRadius:"40px","-moz-border-radius":"40px",boxShadow:"5px 5px 25px rgba(0, 0, 0, 0.3)"}),v=$("<canvas></canvas>").attr({width:160,height:160}).css({position:"absolute",width:"80px",height:"80px",bottom:"20px",right:"5px",zIndex:51,display:"none"});$("body").append(v).append(g).append(q);
return function(){function t(da,A){var fa=new Image;$(fa).load(function(){K.drawImage(fa,256*da,256*A);L++;if(18==L){u.drawImage(k[0],0,0,2048,1024);if(!D){D=wmaGlobe3d(v[0],g[0]);if(!D)return;D.setLatLon(Q.lat,Q.lon);q.fadeIn(200);v.fadeIn(200);D.mapContext=u;D.tmap=k[0];D.tmapContext=K;D.omap=z[0];D.omapContext=z[0].getContext("2d");D.updateTiles=function(){D.mapContext.clearRect(0,0,2048,1024);D.mapContext.drawImage(D.tmap,0,0,2048,1024);D.updateTexture();D.draw()};D.updateKML=function(){D.mapContext.clearRect(0,
0,2048,1024);D.mapContext.drawImage(D.tmap,0,0,2048,1024);D.mapContext.save();D.mapContext.globalAlpha=.5;ka(768,0,0,1536,768,D.omapContext);D.mapContext.drawImage(D.omap,0,0,2048,1024);D.mapContext.restore();D.updateTexture();D.draw()}}B.canvas?D.updateKML():D.updateTiles()}}).attr("src",wma_tilesets[O].getTileURL(A,da,0,!0))}var m,p,L=0,K=k[0].getContext("2d"),u=g[0].getContext("2d");K.clearRect(0,0,1536,768);for(m=0;6>m;++m)for(p=0;3>p;++p)t(m,p)}}();c=/([\d+-.]+)_([\d+-.]+)_([\d]+)_([\d]+)/;c.test(a)&&
...
oa);var k='<select id="wmaLabelSet">';U in wikiminiatlas_sites&&(k+='<option value="'+U+'" selected="selected">'+wikiminiatlas_sites[U]+"</option>");U!==E&&E in wikiminiatlas_sites&&(k+='<option value="'+E+'">'+wikiminiatlas_sites[E]+"</option>");if(U in wikiminiatlas_sites||E in wikiminiatlas_sites)k+="<option disabled></option>";for(g in wikiminiatlas_sites)"commons"!==g&&(k+='<option value="'+g+'">'+wikiminiatlas_sites[g]+"</option>");var z=l.addGroup([["LANG",k+"</select>"],["commons",strings.commons[E]]],
function(t){"LANG"==t&&(t=z.items.LANG.find("select option:selected").val());Ga(t)},"commons"==U?"commons":"LANG",oa);z.items.LANG.find("select").click(function(t){t.stopPropagation()}).change(function(t){z.items.LANG.click()});l.addSep();var q={};for(g=0;g<wma_tilesets.length;++g)k=wma_tilesets[g].globe,q[k]=q[k]||[],q[k].push([g,strings[wma_tilesets[g].name][E]||""]);h='<select id="wmaGlobeSet">';for(g in q)h+='<option value="'+g+'"',g==va&&(h+='selected="selected"'),h+=">"+strings["map"+g][E]+
...
$("#wma_widget").append(l.div.css({right:"40px",top:"26px"}));a=strings.dyk[E];var y=$("<div></div>").html(a[Math.floor(Math.random()*a.length)]).addClass("news");Fa=$("<div></div>").css({position:"absolute",top:"30px",left:"60px",zIndex:100,fontSize:"40px",color:"white",textShadow:"1px 1px 5px black",fontWeight:"bold"}).appendTo("#wma_widget");wmaNews[0]=function(){ua(4)};wmaNews[1]=function(){Y?(F=19039255,I=3363704,aa(15),ua(0)):y.text("Sorry, your browser does not support the canvas element! Try FireFox or Google Chrome.")};
$("#wma_widget").append(y);y.click(function(){y.fadeOut()});setTimeout(function(){y.fadeOut()},1E4);$a=$("#scalelabel");ab=$("#scalebar");document.getElementById("button_target");wa=document.getElementById("wma_settings");$("#wmaLabelSet").change(function(){Ga(this.value)});$("#wmaSetSizeOverlay").change(function(){a:{var g=strings.sover[E].site,k=this.value;if("+"==k&&(k=prompt("Enter article title to use as overlay"),!k))break a;"*"==k&&(k=d.page,g=d.lang);"-"==k?(J.shown=!1,J.canvas.fadeOut(200)):
(Va(g,k),e())}});$("#button_plus").on("mousedown",Wa);$("#button_minus").on("mousedown",Xa);$("#button_target").on("click",lb);$("#button_kml").on("click",S);$(document).on("keydown",kb);$(document).on("contextmenu",function(){return!1});$("body").on("dragstart",function(){return!1});$("#wma_map").click(function(g){g=sa(g);if(g.x!=pa.x||g.y!=pa.y)return!1});x();bldg3d=$('<canvas class="wmakml"></canvas>').attr({width:H*N,height:G*N}).css({zIndex:19,opacity:.75,width:H+"px",height:G+"px"}).appendTo($(ha));
...
$("#wma_widget").mouseover(function(g){if(g.metaKey||g.ctrlKey)if(g.target.href&&eb.test(g.target.href)){g=RegExp.$1;var k=RegExp.$2;$.getJSON("https://"+g+".wikipedia.org/api/rest_v1/page/summary/"+k,function(z){$("#synopsistext").css({direction:z.dir,"padding-left":"0.5em","padding-right":"0.5em"}).html(z.extract_html);if("thumbnail"in z){var q=new Image,v=z.thumbnail.width/2,t=z.thumbnail.height/2,m="ltr"==z.dir?"left":"right";$(q).css({width:v+"px",height:t+"px",position:"absolute",bottom:"0.5em"}).css(m,
"10px").load(function(){$("#synopsistext").css("padding-"+m,v+20+"px").append($(q))}).attr("src",z.thumbnail.source)}$("#synopsis").fadeIn("slow");setTimeout(function(){var p=$("#synopsistext").outerHeight(!0),L=G/2;$("#synopsis").animate({height:p<L?p:L})},500)})}else $("#synopsis").fadeOut("fast")});$("#synopsis").click(function(g){g.target==this&&$("#synopsis").fadeOut("fast")});if(window.postMessage&&($(window).on("message",nb),window!=window.top))try{window.parent.postMessage("request","*")}catch(g){}})()}
...
wmaMenu.prototype.addItem=function(f,d,e){d=d||function(){};var w=this,x=$("<div></div>").addClass("wmamenuitem").html(f).css("direction",!0===e?"rtl":"").mouseenter(function(){x.css("background-color","#AAA")}).mouseleave(function(){x.css("background-color","")}).click(function(){d();w.close()}).appendTo(this.div);return x};
...
wmaMenu.prototype.addCheck=function(f,d,e,w){function x(){e?T.addClass("wmamenuchecked"):T.removeClass("wmamenuchecked")}var T=this.addItem(f,function(){e=!e;x();d(e)},w);d=d||function(){};x();return{toggle:function(S){void 0===S&&(e=!e,x())},check:x,item:T}};
wmaMenu.prototype.addGroup=function(f,d,e,w){function x(X){if(X!=ka){for(var ra in S)ra==X?S[ra].addClass("wmamenuselected"):S[ra].removeClass("wmamenuselected");ka=X}}function T(X,ra){S[X]=Ha.addItem(ra,function(){x(X);d(X)},w)}var S={},Ha=this,ka=-1;e=e||0;d=d||function(){};for(var P=0;P<f.length;++P)"object"===typeof f[P]?T(f[P][0],f[P][1]):T(P,f[P]);x(e);return{items:S,select:x,current:function(){return ka},hide:function(){for(var X in S)S[X].hide()},show:function(){for(var X in S)S[X].show()}}};
